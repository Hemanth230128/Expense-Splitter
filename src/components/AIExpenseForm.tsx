...
const deriveDescriptionFromInput = (text: string): string => {
  ...
  const forMatch = normalized.match(
    /\bfor\s+([^,.]+)(?:\s+for\s+|\s+split\s+|\s+among\s+|\s+between\s+|$)/i
  );
...
const parseCustomSplitsFromText = (
  ...
  for (const member of members) {
    ...
    const patterns = [
      new RegExp(
        `(?:rs\\.?|inr|\\$)\\s*(\\d+(?:\\.\\d+)?)\\s*(?:for|to|by)?\\s*${escaped}\\b`,
        'ig'
      ),

      new RegExp(
        `${escaped}\\b\\s*(?:owes|pays|paid)\\s*(?:rs\\.?|inr|\\$)?\\s*(\\d+(?:\\.\\d+)?)`,
        'ig'
      ),

      new RegExp(
        `${escaped}\\b\\s*[:=-]\\s*(?:rs\\.?|inr|\\$)?\\s*(\\d+(?:\\.\\d+)?)`,
        'ig'
      ),
    ];
...
export function AIExpenseForm({
  ...
  const handleAIParse = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseData?.error || 'AI parsing failed.'
        );
      }
      ...
      const finalDescription =
        result.description &&
          normalizeName(result.description) !== 'expense'
          ? result.description
          : deriveDescriptionFromInput(input);

      const payer = members.find(
        (m) =>
          normalizeName(m.nickname || '') ===
          normalizeName(result.paidBy || '')
      );

      if (!payer) {
        throw new Error(
          `Could not find member matching: ${result.paidBy}`
        );
      }
      ...
      const participantMembers = result.participants
        ...
        .filter(
          (member): member is (typeof members)[number] =>
            !!member
        );
      ...
      if (customSplitMap.size === 0) {
        const parsedFromText = parseCustomSplitsFromText(
          input,
          members
        );

        parsedFromText.forEach((value, key) =>
          customSplitMap.set(key, value)
        );
      }
      ...
      if (customSplitMap.size > 0) {
        for (const member of members) {
          if (
            customSplitMap.has(member.userId) &&
            !participantMembers.some(
      ...
      const customTotal = round2(
        Array.from(customSplitMap.values()).reduce(
          (sum, value) => sum + value,
          0
        )
      );
      ...
      if (
        customSplitMap.size > 0 &&
        customTotal <= result.amount + 0.01
      ) {
        const hasPayer = participantMembers.some(
          (p) => p.userId === payer.userId
        );

        if (!hasPayer) participantMembers.push(payer);

        const payerExplicitAmount = customSplitMap.get(
          payer.userId
        );

        if (payerExplicitAmount === undefined) {
          const payerShare = round2(
            Math.max(0, result.amount - customTotal)
          );
        ...
        participants = participantMembers.map((member) => ({
          ...
          amount: round2(
            customSplitMap.get(member.userId) ?? 0
          ),
        }));
      } else {
        const participantsWithoutCustom =
          participantMembers.filter(
            (m) => !customSplitMap.has(m.userId)
          );

        const remainingAmount = Math.max(
          0,
          result.amount - customTotal
        );

        const equalFallbackShare =
          participantsWithoutCustom.length > 0
            ? remainingAmount /
            participantsWithoutCustom.length
            : 0;

        participants = participantMembers.map((member) => ({
          userId: member.userId,
          amount: round2(
            customSplitMap.get(member.userId) ??
            equalFallbackShare
          ),
        }));
      }
      ...
      if (
        ...
        const payerIdx = participants.findIndex(
          (p) => p.userId === payer.userId
        );
        ...
        participants[idx] = {
          ...participants[idx],
          amount: round2(
            Math.max(0, participants[idx].amount + diff)
          ),
        };
      }
      ...
      if (participants.length === 0) {
        throw new Error(
          'No valid group members identified as participants.'
        );
      }
      ...
      setDoc(expenseRef, expenseData).catch(async () => {
        errorEmitter.emit(
          'permission-error',
      ...
      toast({
        title: 'AI successfully parsed expense!',
        description: `Recorded "${finalDescription}" for $${result.amount}`,
      });
      ...
      toast({
        ...
        description:
          err.message || 'Try a clearer description.',
      });
    } finally {
      setLoading(false);
    }
      ...
      <DialogContent className="sm:max-w-md bg-card border shadow-2xl">
        ...
        <div className="space-y-4 py-4">
          <p className="text-sm text-muted-foreground">
            ...
            Example: "I paid $45 for pizza for Sarah and Priya."
          </p>
